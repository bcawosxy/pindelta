	<div class="page_title">聯繫我們 - 列表[Deleted]</div><hr>
	<div class="admin_tips">123</div>
	<?php 
		$query = 'select * from contact where status = "Delete" order by `inserttime` desc;';
	...
	?>
	Search: <input id="filter" type="text"/>&nbsp;&nbsp;&nbsp;<a class="btn btn-link" href="<?php echo URL_ADMIN_ROOT.'contact?act=index' ?>">[Issue List]</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-link" href="<?php echo URL_ADMIN_ROOT.'contact?act=deleted' ?>">[Deleted]</a><br><br>
	...
	foreach($contact as $k => $v){
		...
		echo 	'<td>'.$v['last_name'].'</td>
				...
				<td>'.$v['company'].'</td>
				<td>'.$v['inserttime'].'</td>
				<td style="width:100%;text-align:center;" '.$class.'>'.$v['status'].'</td>
