...
function delete_issue(id){
	...
	var confirm = new jBox('Confirm', {
			content : content,
			confirm : function(){
				$.post('<?php echo URL_ROOT.'ajax/contact.php?act=delete'?>', {
					id : id,
				}, function(r) {		
					if(r == 1){
						jbox_success('Success!', '<?php echo URL_ADMIN_ROOT.'contact' ?>');
					}else{
						jbox_error('Fail.');
					}
				});
...
function archive_issue(id){
	...
	var confirm = new jBox('Confirm', {
			content : content,
			confirm : function(){
				$.post('<?php echo URL_ROOT.'ajax/contact.php?act=archive'?>', {
					id : id,
				}, function(r) {		
					if(r == 1){
						jbox_success('Success!', '<?php echo URL_ADMIN_ROOT.'contact' ?>');
...
function recover_issue(id){
	...
	var confirm = new jBox('Confirm', {
			content : content,
			confirm : function(){
				$.post('<?php echo URL_ROOT.'ajax/contact.php?act=recover'?>', {
					id : id,
				}, function(r) {		
					if(r == 1){
						jbox_success('Success!', '<?php echo URL_ADMIN_ROOT.'contact' ?>');
